    ...
    <div className='min-h-screen bg-white border-r'>
      {aToken && <ul className='text-[#515151] mt-5'>
        ...
        <NavLink to={'/doctor-list'} className={({ isActive }) => `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? 'bg-[#F2F3FF] border-r-4 border-primary' : ''}`}>
          ...
          <p className='hidden md:block'>Lista Doctor</p>
        </NavLink>
        <NavLink to={'http://192.168.42.138/clinica_salud/frontend/login.php'} className={({ isActive }) => `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? 'bg-[#F2F3FF] border-r-4 border-primary' : ''}`}>
          <img className='min-w-5' src={assets.patient_icon} alt='' />
          <p className='hidden md:block'>Super Admin</p>
        </NavLink>
      </ul>}
